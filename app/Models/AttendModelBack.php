...
class AttendModel extends Model {
    ...
    protected $allowedFields = [
        ...
        'logout_time','early_logout','over_time','working_time' 
    ];
    ...
    {
        // will check whole row in database
        $query = $this->db->table('attend_tbl')->getWhere($data); 
        return $query->getResult();
    }
    ...
    public function getData($id = null)
    {
        $sql = "SELECT `employee_tbl`.`employee_id`, 
                ...
                `shift_tbl`, `emp_shift_tbl`, `roster_tbl`, `leave_tbl`
    ...
    {
        $sql = "SELECT * FROM attend_tbl WHERE employee_id = ? AND login_date = ?";
        $query = $this->db->query($sql, array($id, $login_date));
        return $query->getRow();
        ...
        // $query = $this->db->table('attend_tbl')->getWhere($data);
        // return $query->getRow();
        // $query = $this->db->table('attend_tbl')->where('employee_id',$id)->get();
